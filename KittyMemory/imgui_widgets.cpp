...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v || it_anim->second.selected_rect > 0.01f)
    {
        const char* mark = "~";
        ImFont* font = g.Font;
        float font_size = g.FontSize * 1.5f;
    
        ImVec2 center = total_bb.Min + ImVec2(square_sz * 0.5f, square_sz * 0.5f);
        ImVec2 text_size = CalcTextSize(mark, NULL, false, font_size);
        ImVec2 text_pos = center - text_size * 0.5f;
    
        text_pos.x -= 1.2f;
        text_pos.y -= 2.7f;
    
        ImVec4 mark_color = ImGui::GetStyleColorVec4(ImGuiCol_CheckMark);
        ImU32 mark_color_u32 = ImGui::GetColorU32(ImVec4(mark_color.x, mark_color.y, mark_color.z, mark_color.w * it_anim->second.selected_rect));
        window->DrawList->AddText(font, font_size, text_pos, mark_color_u32, mark);
    }
